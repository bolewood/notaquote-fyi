...
import { carFigures, carPageFor } from "@/lib/car-pages"
import { differenceWords, estimateDollars, signedDollars } from "@/lib/format"
import { addCarsHref, nationalTeenCars, TEEN_CARS_GUIDE, TEEN_COST_GUIDE, teenCarsByGroup, teenCarsHeadline } from "@/lib/guides"
import { articleJsonLd, ENGINE_UPDATED, GUIDES_IMAGE, pageMetadata } from "@/lib/site-meta"
import { joinNames, SAME_ORDER_NOTE } from "@/lib/state-content"
            ...
            return (
              <tr key={item.car.label} className="border-b border-border/60">
                <th scope="row" className="py-2 pr-3 font-normal">
                  {item.car.pick.year} {page ? <Link href={`/cars/${page.slug}`}>{item.car.model}</Link> : item.car.model}
                </th>
...
export default function TeenCarsGuide() {
  ...
  const cheapFigures = cheapestPage ? carFigures(SERVER_CATALOG, cheapestPage.slug) : null
  const priceyFigures = priciestPage ? carFigures(SERVER_CATALOG, priciestPage.slug) : null
  ...
  return (
    <TrustArticle
      title={GUIDE.title}
      lead="Which popular cars cost the least to insure when you add a new 16-year-old, why, and what that doesn't tell you. Worked out from public insurance claims data, the same way in every state."
      breadcrumbs={
        <Breadcrumbs
          crumbs={[
            { name: "Home", path: "/" },
    ...
    >
      ...
      <p>{teenCarsHeadline(SERVER_CATALOG)}</p>
      <p>
        These are {TEEN_LIST_YEAR} models, about the age of a typical first car, from the site&apos;s lists of popular first
        cars, SUVs, and trucks. The figures are for a typical family: a 40–64-year-old parent with a clean record and full
        coverage, at the national typical price. Your state moves every figure up or down.
      </p>
      <div className="flex flex-wrap gap-3">
        <a href={addCarsHref(top5)} className="btn btn-primary plain">
      ...
      {groups.map((group) => (
        <section key={group.id} aria-labelledby={`list-${group.id}`}>
          ...
          <CarTable cars={group.cars} caption={`${group.label}, cheapest to add a teen with first. National figures, ${TEEN_LIST_YEAR} models.`} />
        </section>
      ))}
      ...
      <h2 id="why">Why these cost less</h2>
      <p>
        Insurers pay out more for some cars than others, and the Highway Loss Data Institute publishes how each model&apos;s
        claims compare with the average car&apos;s. Two things matter most: what it costs to fix the car after a crash, and
        how often its drivers damage other cars or hurt someone.
      </p>
      ...
      <p>
        On these lists, the {joinNames(all.slice(0, 3).map((item) => item.car.model))} cost the least to add a teen with,
        and the {joinNames(all.slice(-3).map((item) => item.car.model))} the most. Cars that are cheap to fix, and whose
        drivers file fewer claims, cost less; cars that cost a lot to repair cost more. A newer car also costs more to
        replace, so an older version of the same model usually costs less.
      </p>

      <h2 id="every-state">The same order in every state</h2>
      <p className="font-medium">{SAME_ORDER_NOTE}</p>
      <p>
        With the {cheapest.car.label}, adding a teen costs about {differenceWords(low.teenCars[0].added.increase)} in{" "}
        <Link href={statePath("OH")}>Ohio</Link>, one of the cheaper states, and about{" "}
        {differenceWords(high.teenCars[0].added.increase)} in <Link href={statePath("FL")}>Florida</Link>, one of the
        priciest. The cheapest car is the same in both. <Link href="/states">Your state&apos;s page</Link> has its own
        figures, its minimum coverage, and its neighbors.
      </p>

      <h2 id="added-or-own">Added to your policy, or their own?</h2>
      <p>
        Most families add a new driver to the policy they already have. The household&apos;s bill goes up, but by much less
        than the teen would pay alone: with the {cheapest.car.model}, about {differenceWords(cheapest.added.increase)} added,
        against about {estimateDollars(cheapest.own.likely)} a year on their own policy.
      </p>
      <p>
        <Link href={TEEN_COST_GUIDE.path}>How much adding a teen driver costs, state by state</Link>.
      </p>
      ...
      <p className="text-sm text-muted-foreground">
        The comparison adds these cars to your list, priced for your own driver and state. <Link href="/methodology">Here&apos;s how we got these numbers</Link>.
      </p>
    </TrustArticle>
  )
}
