...
test("differences read as words in sentences and signs in cells", () => {
  ...
  assert.equal(differenceWords(627, "month"), "$55 more a month", "$630 a year is $52.50 a month, shown as $55")
  assert.equal(monthlyDollars(630), "$55")
  assert.equal(differenceWords(40, "month"), "$5 more a month")
  assert.equal(differenceWords(20, "month"), "about the same")
